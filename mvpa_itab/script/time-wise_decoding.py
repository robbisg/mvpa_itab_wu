...
from test.pystone import Array1Glob
...
subjects = os.listdir('/DATA11/roberto/fmri/data7/Carlo_MDM/')
subjects = [s for s in subjects if str.isdigit(s[0])]
...
for t_ in tasks:
    for ev_ in evidences:
        ...
        for i, subj in enumerate(subjects):
            folder_ = '%s_%s_BETA_MVPA_evidence_%s_balance_ds_1' \
                        % (subj, t_, str(ev_))
             
            fname_ =  '%s_%s_BETA_MVPA_evidence_%s_balance_ds_1_radius_3_searchlight_mean_map.nii.gz' \
                        % (subj, t_, str(ev_))
            
            total_ =  '%s_%s_BETA_MVPA_evidence_%s_balance_ds_1_radius_3_searchlight_total_map.nii.gz' \
                        % (subj, t_, str(ev_))
            ...
            img_name = os.path.join(path, folder_, fname_)
...
def check_total(folder_, total_, task_, subj_, evidence_):
    ...
    img_name = os.path.join(path, folder_, total_)
    img_ = ni.load(img_name)
    ...
    fname_ = '%s_%s_evidence_%s_%s.nii.gz'
    
    fname_save = os.path.join(path, folder_, '%s_%s_evidence_%s_%s.nii.gz' %(subj_, task_, str(evidence_), 't'))
    ni.save(ni.Nifti1Image(t, affine), fname_save)
    fname_save = os.path.join(path, folder_, '%s_%s_evidence_%s_p_chance_%s.nii.gz' %(subj_, task_, str(evidence_), 'p'))
