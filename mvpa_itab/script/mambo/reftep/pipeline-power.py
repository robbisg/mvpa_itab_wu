...
_default_options = {                     
                       ...
                       'loader__bids_win': ['500'],

                       #'estimator__fsel__k': np.arange(50, 15000, 1500),

                       'estimator__clf': [
                           MLPRegressor(hidden_layer_sizes=(1024, 128)),
                           MLPRegressor(hidden_layer_sizes=(1024)),
                           MLPRegressor(hidden_layer_sizes=(512, 64)),
                           SVR(C=1),
                           SVR(C=10),
                           Lasso(alpha=0.5),
                        ],                          
...
for conf in iterator:
    ...
    try:
        a = AnalysisPipeline(conf, name="reftep+power+singleregression").fit(**kwargs)
        a.save()
