...
def analysis(**kwargs):
    ...
    for cond in conditions:
        ...
        directory = os.path.join(path, method, cond, filter_type)
...
def get_clustering(condition, **kwargs):
    """
    Gets the labels from precomputer clustering
    """
    configuration = {
                     'path':'/media/robbis/DATA/fmri/movie_viviana/beta',
                     'filetype' : 'masked',
                     'fname': 'mat_corr_sub_%s.mat',
                     'conditions' : ['movie', 'scramble', 'rest'],
                     'state_res_fname' : "clustering_labels_%s_maxk_%s_%s_%s.pyobj",
    ...
    path_cluster = "%s/%s/%s" % (filetype, method, condition)
